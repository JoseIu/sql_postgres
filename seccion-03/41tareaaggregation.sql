...
SELECT COUNT(*) as users FROM users WHERE email LIKE '%@google.com';
...
SELECT COUNT(*) as users, country FROM users GROUP BY country;
...
SELECT * FROM users WHERE country IN ('Mexico', 'Honduras', 'Costa Rica') ORDER BY country ASC , first_name ASC, last_name ASC;

-- 9. Del query anterior, cuente cuántas personas hay por país
-- Ordene los resultados por País asc

SELECT COUNT(*) as total, country FROM users WHERE country IN ('Mexico', 'Honduras', 'Costa Rica') GROUP BY country ORDER BY country ASC;
